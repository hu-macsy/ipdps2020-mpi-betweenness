/*
 * DistributedKadabra.hpp
 *    Based on KadabraBetweenness.hpp
 *    Authors: Alexander van der Grinten <avdgrinten@hu-berlin.de>
 */
...
class DistributedStateFrame {
  public:
    DistributedStateFrame(const count size) { apx.assign(size, 0); };
    count nPairs = 0;
    count epoch = 0;
...
class DistributedStatus {
  public:
    DistributedStatus(const count k);
    const count k;
    std::vector<node> top;
...
class DistributedSpSampler {
  private:
    const Graph &G;
    const ConnectedComponents &cc;

  public:
    DistributedSpSampler(const Graph &G, const ConnectedComponents &cc);
    void randomPath(DistributedStateFrame *curFrame);
    DistributedStateFrame *frame;
    std::mt19937_64 rng;
...
class DistributedKadabra : public Algorithm {
  ...
  protected:
    ...
    std::vector<std::atomic<DistributedStateFrame *>> epochFinished;
    std::vector<DistributedSpSampler> samplerVec;
    ...
    void computeBetErr(DistributedStatus *status, std::vector<double> &bet,
                       std::vector<double> &errL,
                       std::vector<double> &errU) const;
    bool computeFinished(DistributedStatus *status) const;
    void getStatus(DistributedStatus *status, const bool parallel = false) const;
    void computeApproxParallel(const std::vector<DistributedStateFrame> &firstFrames);
    ...
    void fillResult();
	bool aggregateLocally(count *data);
    bool checkConvergence(DistributedStatus &status);
...
DistributedKadabra::ranking() const {
    assureFinished();
